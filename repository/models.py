...
__all__ = ["UserProfile", "AdminInfo", "UserGroup", "BusinessUnit", "IDC",
           "Tag", "Asset", "Server", "NetworkDevice", "Disk", "NIC",
           "Memory", "AssetRecord", "ErrorLog"]
...
class BusinessUnit(models.Model):
    ...
    contact = models.ForeignKey(UserGroup, verbose_name="业务联系人", related_name="c")  # 运维管理组
    manager = models.ForeignKey(UserGroup, verbose_name="业务管理员", related_name="m")  # 业务联系人组
...
class Asset(models.Model):
    ...
    device_type_id = models.IntegerField(choices=device_type_choice, default=1)
    device_status_id = models.IntegerField(choices=device_status_choice, default=1)
    ...
    business = models.ForeignKey("BusinessUnit", verbose_name="属于的业务线", null=True, blank=True)

    tag = models.ManyToManyField("Tag")

    latest_dte = models.DateTimeField(null=True)
    ...
    def __str__(self):
        return f"{self.idc.name}-{self.cabinet_num}-{self.cabinet_order}"
...
class NetworkDevice(models.Model):
    ...
    device_detail = models.CharField("设备详细配置", max_length=255, null=True, blank=True)
...
class AssetRecord(models.Model):
    ...
    def __str__(self):
        return f"{self.asset_obj.idc.name}-{self.asset_obj.cabinet_num}-{self.asset_obj.cabinet_order}"
